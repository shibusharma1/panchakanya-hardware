                    ...
                    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" id="name" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm" value="{{ old('name', $category->name) }}" required>
                                @error('name')
                            ...
                            <div>
                                ...
                                <input type="text" name="icon" id="icon" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm" value="{{ old('icon', $category->icon) }}" placeholder="e.g. fa-solid fa-home">
                                @error('icon')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                @if($category->image)
                                    <div class="mt-2 mb-3">
                                        <img src="{{ asset('storage/' . $category->image) }}" alt="Current Image" class="w-20 h-20 object-cover rounded-lg border border-gray-200 shadow-sm">
                                    </div>
                                @endif
