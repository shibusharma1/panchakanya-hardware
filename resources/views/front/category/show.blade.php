        ...
        <div class="pt-6 pb-24 lg:grid lg:grid-cols-4 lg:gap-x-10">
                ...
                <aside class="hidden lg:block animate-on-scroll">
                    <div class="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
                        ...
                        <ul class="space-y-3">
                            <li>
                                <a href="{{ route('products.index') }}" class="flex items-center text-gray-600 hover:text-primary-red transition-colors group">
                                    <span class="w-2 h-2 rounded-full bg-gray-300 mr-3 group-hover:bg-primary-red transition-colors"></span>
                                    All Categories
                                </a>
                            </li>
                            @if(isset($globalCategories))
                                @foreach($globalCategories as $cat)
                                    <li>
                                        <a href="{{ route('category.show', $cat->slug) }}" class="flex items-center {{ $cat->id == $category->id ? 'text-primary-red font-bold' : 'text-gray-600 hover:text-primary-red' }} transition-colors group">
                                            <span class="w-2 h-2 rounded-full {{ $cat->id == $category->id ? 'bg-primary-red' : 'bg-gray-300 group-hover:bg-primary-red' }} mr-3 transition-colors"></span>
                                            {{ $cat->name }}
                                        </a>
                                    </li>
                                @endforeach
                    ...
                    <div class="grid grid-cols-1 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 gap-x-8">
                        ...
                        @empty
                            <div class="col-span-3 text-center py-20 bg-white rounded-2xl shadow-sm animate-on-scroll">
                                ...
                                <p class="text-xl text-gray-500 font-medium">No products found in this category.</p>
